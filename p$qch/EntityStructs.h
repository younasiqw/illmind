...
public:
	...
	int *GetSequence()
	{
		static int m_nSequence = GET_NETVAR(XorStr("DT_CSPlayer"), XorStr("m_nSequence"));
		return GetFieldPointer<int>(m_nSequence);
	}

	float *GetCycle()
	{
		static int m_flCycle = GET_NETVAR(XorStr("DT_CSPlayer"), XorStr("m_flCycle"));
	...
	{
		return GetVelocity()->Length2D() > 0.75f && (*(int*)this->GetFlags() & *(int*)SSDK::EntityFlags::FL_ONGROUND);
	}
	...
	void SetAngle2(SSDK::Vector desired_angle)
	{
		typedef void(__thiscall* SetAngleFn)(void*, const SSDK::Vector &);
		static SetAngleFn SetAngle = (SetAngleFn)((DWORD)GetModuleHandle("client.dll") + 0x1C2AC0);
		SetAngle(this, desired_angle);
	}
};
